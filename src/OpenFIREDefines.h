...
// #define USES_ANALOG // [ESP32_PORT] definito in platformio.ini
  ...
  // Leave this uncommented to support a Wii Nunchuk connected via I2C (on the
  // "Peripherals SDA/SCL" pins) as an extra analog stick + 2 extra buttons
  // (Z = dup. Button A, C = Button C). [MANCO] ported from the RP2040 build.
// #define USES_NUNCHUK // [ESP32_PORT] definito in platformio.ini

  // [MANCO] Upstream tears down the ESP-NOW radio entirely whenever the gun
  // is connected via USB cable (HID goes out over USB either way, wired or
  // not). Leave this uncommented so that, when cabled, the radio instead
  // stays up on a fixed channel (no dongle search - that'd be pointless with
  // HID already on USB) purely to let wireless accessories - the pedal
  // today, others later - keep pairing/reconnecting to this gun. Requires
  // OPENFIRE_WIRELESS_ENABLE. See HandleWirelessAccessories() in main.cpp.
// #define MANCO_WIRELESS_ACCESSORIES_WITH_CABLE // [ESP32_PORT] definito in platformio.ini
