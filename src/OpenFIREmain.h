...
#ifdef USES_NUNCHUK
// [MANCO] Wii Nunchuk stick/buttons polling - see OpenFIRENunchuk.h and the
// definitions near the end of main.cpp (after the AnalogStickPoll block).
void NunchukStickPoll();
// [MANCO] Re-centers/neutralizes the stick output when the Nunchuk is hot-
// unplugged mid-session, instead of leaving it stuck on its last reading.
void NunchukStickRelease();
void NunchukButtonsPoll();
#endif // USES_NUNCHUK
#if defined(ARDUINO_ARCH_ESP32) && defined(OPENFIRE_WIRELESS_ENABLE)
// [MANCO] Pairs/keeps alive wireless accessories (pedal today, extensible to
// others later) over ESP-NOW on a fixed channel - independent of whether the
// gun's own HID is going out over USB or over the wireless link to a dongle.
// See the "cable + wireless accessories" block in setup().
void HandleWirelessAccessories();
// [MANCO] Runs HandleWirelessAccessories() in its own FreeRTOS task instead
// of inline, so its blocking pedal-search calls never delay USB/HID boot.
// This is what setup() actually calls; see main.cpp for details.
void StartWirelessAccessoriesTask();
